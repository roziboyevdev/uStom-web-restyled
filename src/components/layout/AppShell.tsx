...
export const AppShell: React.FC<AppShellProps> = ({ children, currentView, onNavigate }) => {
  ...
  return (
    <div className="min-h-screen bg-[#FAF9F6] text-[#1C1C1C] flex flex-col font-sans transition-colors duration-300">
      
      {/* GLOBAL HEADER BAR */}
      <header className={`sticky top-0 z-50 w-full transition-all duration-300 ${
        scrolled 
          ? 'bg-[#FAF9F6]/90 backdrop-blur-md shadow-premium border-b border-[#1C1C1C]/10' 
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
            ...
            {menuItems.map((item) => {
              ...
              return (
                ...
                >
                  {item.label}
                  {isActive && (
                    <span className="absolute bottom-0 left-0 w-full h-0.5 bg-[#1C1C1C]" />
                  )}
                </button>
              );
            })}
          ...
          <div className="hidden md:flex items-center gap-4">
            {user ? (
              <div className="flex items-center gap-3 bg-white/80 hover:bg-white p-1.5 pr-4 rounded-none transition-all duration-200 border border-[#1C1C1C]/10">
                <div 
                ...
                >
                  <img 
                    src={user.avatar} 
                    alt={user.name} 
                    className="w-8.5 h-8.5 rounded-none object-cover ring-2 ring-[#1C1C1C]/10 shadow-sm"
                  />
                  <div className="flex flex-col text-left">
                    <span className="text-xs font-bold text-slate-800 line-clamp-1">{user.name}</span>
                    <span className="text-[10px] font-semibold text-slate-450 uppercase tracking-widest">Kabinet</span>
                  </div>
                </div>
              ...
              <button
                ...
                className="inline-flex items-center gap-2 border border-[#1C1C1C] text-[#1C1C1C] hover:bg-[#1C1C1C] hover:text-[#FAF9F6] text-[10px] uppercase font-bold tracking-[0.22em] px-5 py-2.5 transition-all duration-300 active:scale-98 cursor-pointer"
              >
          ...
          <div className="flex items-center gap-3 md:hidden">
            {user && (
              <div 
                onClick={() => handleNavClick('profile')}
                className="w-9 h-9 rounded-xl overflow-hidden ring-2 ring-emerald-500/20 shadow-sm cursor-pointer"
              >
            ...
            <button
              ...
              className="p-2 text-slate-600 hover:text-emerald-500 hover:bg-slate-100 rounded-xl transition-all cursor-pointer"
            >
          ...
          <div className="flex flex-col gap-4 border-t border-slate-100 pt-6">
            {user ? (
              <div className="flex flex-col gap-3">
                ...
                >
                  <img src={user.avatar} alt={user.name} className="w-10 h-10 rounded-lg object-cover ring-2 ring-emerald-500/15" />
              ...
              <button
                onClick={() => handleNavClick('auth')}
                className="flex items-center justify-center gap-2 w-full bg-slate-900 text-white hover:bg-emerald-600 py-3 rounded-xl text-sm font-semibold transition-all shadow-sm cursor-pointer"
              >
                Profilga kirish
                <ArrowRight className="w-4 h-4" />
              </button>
      ...
      <nav className="md:hidden fixed bottom-4 left-4 right-4 bg-white/90 backdrop-blur-md shadow-2xl rounded-2xl border border-slate-100/80 p-2 flex items-center justify-around z-40">
        {menuItems.map((item) => {
          const isActive = currentView === item.view || (item.view === 'courses' && currentView === 'course-detail') || (item.view === 'professors' && currentView === 'professor-detail');
          const Icon = item.icon;
          return (
            <button
              ...
              className={`flex flex-col items-center justify-center py-1.5 px-3 rounded-xl transition-all cursor-pointer ${
                isActive ? 'text-emerald-500 bg-emerald-50/50 font-bold' : 'text-slate-500'
              }`}
            >
        ...
        <button
          ...
          className={`flex flex-col items-center justify-center py-1.5 px-3 rounded-xl transition-all cursor-pointer ${
            currentView === 'profile' || currentView === 'auth' ? 'text-emerald-500 bg-emerald-50/50 font-bold' : 'text-slate-500'
          }`}
        >
      ...
      <footer className="bg-[#1C1C1C] text-[#FAF9F6]/70 border-t border-[#1C1C1C]/20 py-16 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-12">
          
          <div className="md:col-span-2 flex flex-col gap-5">
            <Logo dark className="pointer-events-none" />
            <p className="text-sm text-[#FAF9F6]/60 max-w-md mt-2 leading-relaxed font-light">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-3 text-sm font-light">
              <button onClick={() => onNavigate('courses')} className="text-left text-[#FAF9F6]/60 hover:text-white transition-colors">Implantologiya</button>
              <button onClick={() => onNavigate('courses')} className="text-left text-[#FAF9F6]/60 hover:text-white transition-colors">Ortodontiya & Gnatologiya</button>
              <button onClick={() => onNavigate('courses')} className="text-left text-[#FAF9F6]/60 hover:text-white transition-colors">Estetik Stomatologiya</button>
              <button onClick={() => onNavigate('courses')} className="text-left text-[#FAF9F6]/60 hover:text-white transition-colors">Mikroskopik Endodontiya</button>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-3 text-sm font-light">
              ...
              <div className="flex items-center gap-2">
                <MessageSquare className="w-4 h-4 text-white opacity-80" />
                <span>Telegram: @ustom_support</span>
              </div>
              <div className="flex items-center gap-2.5">
                <MapPin className="w-4.5 h-4.5 text-white opacity-80 shrink-0" />
        ...
        <div className="max-w-7xl mx-auto border-t border-white/10 mt-12 pt-8 flex flex-col sm:flex-row items-center justify-between text-xs text-[#FAF9F6]/40 gap-4">
